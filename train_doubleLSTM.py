...
def predict(day_before):
    day_before = np.reshape(
        day_before, (day_before.shape[0], n_features, n_step))
...
def generate_res_infection_lst(cities_prediction):
    ...
    for city_prediction in cities_prediction:
        ...
        for region in city_prediction:
            for date in region:
                res_infection.append(int(round(date[0][0])) if int(
                    round(date[0][0])) > 0 else 0)
...
if __name__ == "__main__":
    ...
    model.add(LSTM(units=128, input_shape=(
        X_test.shape[1], X_test.shape[2]), return_sequences=True))
    ...
    submission.to_csv('data/test_submission_double_LSTM.csv',
